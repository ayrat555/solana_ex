defmodule Solana.CompactArray do
  ...
  def encode_length(length) when Bitwise.bsr(length, 7) == 0, do: [encode_bits(length)]

  def encode_length(length) do
    [Bitwise.bor(encode_bits(length), 0x80) | encode_length(Bitwise.bsr(length, 7))]
  end

  defp encode_bits(bits), do: Bitwise.band(bits, 0x7F)
  ...
  def decode_length(<<elem, _::binary>>, size) when Bitwise.band(elem, 0x80) == 0 do
    decode_bits(elem, size)
  end

  def decode_length([elem | _], size) when Bitwise.band(elem, 0x80) == 0 do
    decode_bits(elem, size)
  end

  def decode_length(<<elem, rest::binary>>, size) do
    Bitwise.bor(decode_bits(elem, size), decode_length(rest, size + 1))
  end
  ...
  defp decode_bits(bits, size), do: bits |> Bitwise.band(0x7F) |> Bitwise.bsl(7 * size)
